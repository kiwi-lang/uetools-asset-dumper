...
# This does not work
#
# From https://gist.github.com/andyneff/314172b5bdcb981c030ff0aa6bf2fa4a
# 
import logging
import re
import collections
...
def string_searcher(data, search_long=False):
    ...
    for i in re.finditer(pattern, data):
        ...
        if re.search(
            "[\x00-\x1f\x80-\xff]", data[start + 4 : start + string_length + 3]
        ):
            continue

        strings.append(
            (i.start(), string_length, data[start + 4 : start + 4 + string_length - 1])
        )
...
def sdiff(*a):
    for i, d in enumerate(zip(*a)):
        print("{}:" + "\t{}" * len(a)).format(
            i, *(ord(x) if isinstance(x, str) else x for x in d)
        )
...
class Uasset(object):
    ...
    def peek(self, peek_size):
        ...
        return self.data.buf[self.data.tell() : self.data.tell() + peek_size]
    ...
    def load(self, filename):
        ...
        self.logger.debug("head: {} @ {}".format(self.count, self.data.tell()))
        ...
        print('Warning: ', len(self.footer) == 4) 
...
class DataTable(Uasset):
    def read_header(self):
        ...
        self.read_lut()
        self.logger.debug("l {}".format(self.data.tell()))
    ...
    def read_item(self):
        ...
        x = self.read_uint32()
        self.logger.warning("Item {} {}".format(item_key, self.data.tell()))
...
class RokhDataTable(DataTable):
    def __init__(self, filename=None):
        ...
        self.handles.update(
            {
                "ByteProperty": self.read_byte_property,
    ...
    def read_text_property(self):
        ...
        self.logger.debug("tps: {} @ {}".format(size, self.data.tell() - 8))
        ...
        if size >= 18:
            ...
            q = self.read_string()
            self.logger.debug("tpe: {}".format(self.data.tell()))
            return q
